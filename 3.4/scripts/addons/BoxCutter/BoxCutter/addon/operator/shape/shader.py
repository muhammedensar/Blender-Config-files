...
from ... utility import addon, screen, object
...
class shape:
    ...
    def __init__(self, ot, context):
        ...
        try:
            # dat = bc.shape.to_mesh()
            dat = object.mesh_duplicate(bc.shape, context.evaluated_depsgraph_get())
        except:
        ...
        if ot.shape_type == 'CIRCLE':
            bmesh.ops.dissolve_limit(bm, angle_limit=0.008, use_dissolve_boundaries=True, verts=bm.verts, edges=bm.edges)
        ...
        for mod in bc.shape.modifiers:
            if mod.type == 'SOLIDIFY' and bc.lattice.dimensions[2] > 0.001:
                color[3] = color[3] * 0.5
                break

        wire_color = preference.color.wire if not preference.behavior.show_shape else preference.color.show_shape_wire
