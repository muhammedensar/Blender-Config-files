...
from ... utility import active_tool, addon, object, ray, modifier, modal
...
class BC_OT_display_snap(Operator):
    ...
    def invoke(self, context, event):
        ...
        bc.snap.display = False if self.system == 'MAC' and not event.ctrl else True
        ...
        if bc.snap.display and preference.behavior.snap:
            hit = False
            ...
            if surface and context.active_object and context.active_object.select_get() and context.active_object.type == 'MESH':
                ...
                bc.snap.mesh = bpy.data.meshes.new_from_object(bc.snap.object.evaluated_get(bpy.context.evaluated_depsgraph_get()))
            ...
            hit, _, self.normal, self.index = ray.cast.objects(*self.mouse, mesh=bc.snap.mesh, snap=True)
            ...
            if (not hit or align_to_view) and context.selected_objects: # view aligned
                ...
                _, _, self.normal, self.index = ray.cast.objects(*self.mouse, mesh=bc.snap.mesh, snap=True)
    ...
    def modal(self, context, event):
        ...
        if not held or context.region != self.original_region or bc.running:
            self.exit()

            return {'CANCELLED'}

        elif bc.snap.display and preference.behavior.snap:
            if event.type == 'MOUSEMOVE' and not self.highlight or bc_reset :
                hit, _, self.normal, self.index = ray.cast.objects(self.mouse.x, self.mouse.y, mesh=bc.snap.mesh, snap=True)
