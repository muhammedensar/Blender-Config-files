...
from ... utility import active_tool, addon, data, lattice, mesh, modal, modifier, object, ray
...
class BC_OT_draw_shape(Operator):
    ...
    mode: EnumProperty(
        ...
        items = [
            ...
            ('EXTRACT', 'Extract', 'Modal Shortcut: Y', icon.id('black'), 6),
            ('MAKE', 'Make', 'Modal Shortcut: A', icon.id('grey'), 7)],
        default = 'CUT')
    ...
    def invoke(self, context, event):
        ...
        if preference.keymap.allow_selection and preference.keymap.edit_disable_modifiers:
            if self.original_mode == 'EDIT_MESH':
                if not bc.snap.display or not bc.snap.hit:
        ...
        if preference.surface == 'OBJECT':
            hit, self.ray['location'], self.ray['normal'], *_ = ray.cast.objects(*self.mouse['location'])
    ...
    def execute(self, context):
        ...
        if bc.shape.dimensions[2] < preference.shape.lazorcut_limit and not self.repeat:
            ...
            # too_thin = dimension_z < lazorcut_limit and not self.extruded
            aligned = self.mode == 'KNIFE' and self.align_to_view

            if self.shape_type != 'NGON' and not aligned:
    ...
    def modal(self, context, event):
        ...
        if within_region_3d:
            ...
            if event.type == 'LEFTMOUSE':
                ...
                if not (event.alt and preference.keymap.alt_preserve):
                    if event.value == 'RELEASE':
            ...
            # 1, 2, 3
                ...
                if event.value == 'RELEASE':
                    ...
                    if self.operation != 'SOLIDIFY':
                        ...
                        modal.mirror.shape(self, context, event, to=axis[event.type], flip=event.shift)
                    else:
                        for mod in bc.shape.modifiers:
                            if mod.type == 'SOLIDIFY':
            ...
            # F
                ...
                if event.shift and self.shape_type != 'NGON':
                    if event.value == 'RELEASE':
                        modal.flip.shape(self, context, event)
                ...
                if event.value == 'RELEASE':
                    if self.shape_type == 'NGON' and self.operation == 'DRAW' and event.type == 'Z' and event.ctrl:
                        ...
                        if event.type == 'X':
                            ...
                            modal.mode.change(self, context, event, to='EXTRACT')
