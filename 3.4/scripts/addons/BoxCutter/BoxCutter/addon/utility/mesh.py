...
def remove_point(ot, context, event, index=-1, fill=True):
    ...
    if len(bm.verts) > 2 or index != -1:
        ...
        if fill and len(bm.verts) > 2 and bc.cyclic:
            bm.verts.ensure_lookup_table()
...
def add_point(ot, context, event):
    ...
    if len(bm.verts) > 2 and bc.cyclic:
        bm.edges.ensure_lookup_table()
...
def offset(ot, context, event):
    ...
    if ot.view3d['location'].z > ot.start['extrude']:
        matrix = ot.start['matrix'] @ Matrix.Translation(Vector((0, 0, ot.view3d['location'].z + offset)))
        ...
        matrix = ot.start['matrix'] @ Matrix.Translation(Vector((0, 0, ot.start['extrude'] + offset)))
        bc.shape.matrix_world.translation = matrix.translation
...
def extrude(ot, context, event, extrude_only=True):
    ...
    if not extrude_only:
        ...
        for point in points:
            point.co.z = location_z if location_z < 0 else -0.0001
...
def knife(ot, context, event):
    ...
    too_thin = dimension_z < lazorcut_limit and not ot.extruded
    aligned = not ot.extruded and ot.align_to_view

    if not ot.extruded or too_thin or aligned:
        return
    ...
    for pair in zip(targets, overrides):
        ...
        bm = bmesh.new()
        applied_shape = bpy.data.meshes.new_from_object(bc.shape.evaluated_get(bpy.context.evaluated_depsgraph_get()))
...
class create:
    ...
    def shape(ot, context, event):
        ...
        if ot.shape_type == 'BOX':
            ...
            ot.geo['indices']['top_edge'] = [0]
            ot.geo['indices']['mid_edge'] = [1]
            ot.geo['indices']['bot_edge'] = [2]
            ...
            ot.geo['indices']['top_edge'] = [0]
            ot.geo['indices']['mid_edge'] = []
            ot.geo['indices']['bot_edge'] = []

        dat = bpy.data.meshes.new(name='Cutter')

        dat.from_pydata(verts, edges, faces)

        if ot.shape_type == 'CIRCLE' and bc.snap.hit:
