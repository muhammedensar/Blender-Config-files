...
def custom(ot, context, event, axis=None):
    ...
    if bc.snap.hit:
        hit, ot.ray['location'], ot.ray['normal'], index = ray.cast.objects(*ot.mouse['location'], mesh=bc.snap.mesh, axis=current[axis])
...
def screen(ot, context, event):
    ...
    if not bc.snap.hit:
        ...
        modifier.apply(box, mod=mod, hard_apply=False)
