...
def cycle(ot, context, event, index=1, custom=None):
    ...
    if len([obj for obj in bc.collection.objects if obj.type == 'MESH']) < 2:
        return
    ...
    if not custom and not bc.shape.bc.applied and not bc.shape.bc.applied_cycle:
        ...
        modifier.apply(bc.shape, exclude=[mod for mod in bc.shape.modifiers if mod.type in keep_modifiers])
    ...
    flip_join = ot.mode == 'JOIN' and preference.behavior.join_flip_z
