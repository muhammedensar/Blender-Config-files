...
from . import active_tool, addon, object, mesh
...
def sort(ot, obj, force=False):
    ...
    if preference.behavior.sort_modifiers or force:
        ...
        if preference.behavior.sort_decimate: mod_types.append('DECIMATE')
        if preference.behavior.sort_remesh: mod_types.append('REMESH')
        if preference.behavior.sort_subsurf: mod_types.append('SUBSURF')
...
def apply(obj=None, mod=None, visible=False, modifiers=[], exclude=[], type='NONE', hard_apply=True, kitops=False):
    bc = bpy.context.window_manager.bc

    mode = active_tool().mode
    mods = []
    ...
    if mod:
        ...
        for m in obj.modifiers:
            if m not in exclude:
                if m in modifiers or not len(modifiers):
    ...
    if mods:
        ...
        has_group = lambda mod: hasattr(mod, 'vertex_group') and mod.vertex_group
        screw = lambda mod: mod.type == 'SCREW'
        explicit = [mod.name for mod in mods if hard_apply or has_group(mod) or screw(mod)]
        names = [mod.name for mod in mods if mod.name not in explicit]

        if obj.data.users > 1 and (explicit or names):
            obj.data = obj.data.copy()
        ...
        if explicit:
            ...
            for name in explicit:
                bpy.ops.object.modifier_apply(c, apply_as='DATA', modifier=name)
...
class duplicate:
    ...
    def UV_PROJECT(self, mod):
        ...
        for index, proj in enumerate(mod.projectors):
            self.modifier.projectors[index].object = proj.object
