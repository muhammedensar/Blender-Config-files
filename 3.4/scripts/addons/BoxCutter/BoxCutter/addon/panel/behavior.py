...
from .. utility import active_tool, addon, names, vertice_presets, array_presets, width_presets, segment_presets, angle_presets
...
class BC_PT_helper(Panel):
    ...
    def poll(cls, context):
        return active_tool().idname == 'BoxCutter' and context.region.type == 'WINDOW'
    ...
    def label_row(self, row, path, prop, label=''):
        ...
        if prop in {'circle_vertices', 'ngon_snap_angle', 'array_count', 'bevel_width', 'bevel_segments'}:
            ...
            pointer = '.behavior.' if prop == 'ngon_snap_angle' else '.shape.'
